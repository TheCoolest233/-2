	...
	<table class="list">
	 ...
	 <c:forEach begin="0" end="${list.size()-1}" var="i">
		<tr>
			...
			<td>
				<a class="fa fa-remove" title="删除" href="MessageServlet1?message_id=${list[i].message_id}&service=deleteLjMessage&name=1" onclick="confirmDelete(1)"></a>
			</td>
		</tr>
</c:forEach>
</c:if>
	...
	<c:if test="${count %5!=0}">
		...
		<div>
			...
			<c:if test="${page>1 }">
				...
				<a href="MessageServlet1?page=${page-1 }&service=outShow">上一页</a>
				</li>
				</c:if>
				<li class="paginate_button active"><a href="#">${page }</a></li>
				<c:if test="${page<zpage }">
				<li class="paginate_button next disabled">
				<a href="MessageServlet1?page=${page+1 }&service=outShow">下一页</a>
				</li>
				</c:if>
			</ul>
		</div>
